...
int main(int argc, char **argv) {
    ...
    std::vector<int> ans_int(val.size());
    std::transform(val.begin(), val.end(), ans_int.begin(), [](int x) { return 2 * x; });
    std::vector<std::string> ans(val.size());
    std::transform(ans_int.begin(), ans_int.end(), ans.begin(), [](int x) { return std::to_string(x); });
